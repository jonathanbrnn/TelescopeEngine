...
void Grid::Start() {
    ...
    for (int i = 2; i < cell_threshold; i++) {
    ...
    SDL_Renderer* renderer = managerHub->Get_Renderer(); 
    ...
    for (int i = 0; i < static_cast<int>(screen_height / common_divisor); i++) {
        ...
        for (int j = 0; j < static_cast<int>(screen_width / common_divisor); j++) {
            Cell* current_cell = new Cell(renderer, "cell" + to_string(i) + to_string(common_divisor + j), common_divisor * j, common_divisor * i, 0, common_divisor, common_divisor, 0);
